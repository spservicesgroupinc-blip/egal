...
class GeminiService {
  ...
  async draftMotion(
    ...
    const context = this.formatCaseFiles(files);

    const prompt = [
      `Draft a professional Indiana civil law motion regarding: ${topic}`,
  ...
  async chatWithExpert(
    ...
    const fullMessage = context
      ? `CASE DATA (do NOT repeat unless asked):\n${context}\n\nUSER: ${message}`
      : message;
    ...
    const runChat = async (model: string) => {
      const chat = this.ai.chats.create({
        ...
        config: {
          systemInstruction:
            "You are HoosierLaw – knowledgeable, professional Indiana civil law assistant handling all types of civil matters including contracts, torts, property, family, and more. Be concise, accurate, cite authority when possible.",
          temperature: 0.45,
        },
        history,
      });

      const res = await chat.sendMessage({ message: fullMessage });
      this.trackUsage(res);
